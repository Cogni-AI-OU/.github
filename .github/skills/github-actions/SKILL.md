...
---
name: github-actions
description: When a GitHub Actions workflow fails in the current repository or pull request, diagnose the root cause by retrieving run statuses and logs using available MCP tools or the gh CLI.
---

This skill enables autonomous diagnosis of GitHub Actions failures, preferring MCP server tools for efficient summaries and falling back to the gh CLI for direct log access when needed.
...
1. Detect available tools for diagnosis.
   ...
   Prioritize MCP tools (e.g., `list_workflow_runs`, `summarize_job_log_failures`) if present in the toolset — they provide the most token-efficient summaries.  

...
- **Micro-State Serialization Mandate**: When executing pure-logic, no-script solutions for complex state machines or positional grids (e.g. Sudoku, board constraints), NEVER dump the monolithic visual state (like the entire 9x9 grid) continuously, as it forces autoregressive looping. Instead, serialize your mental scratchpad: explicitly maintain an itemized queue (e.g., `Cell(X,Y): Remaining [A,B]`) with explicit refresh after every placement and CoV (Chain-of-Verification) check against all affected rows/columns/boxes to force token-by-token verification.

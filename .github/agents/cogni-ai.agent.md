...
- **Anti-Prose State Tracking (Language-Drift Containment)**: Do not use natural language paragraphs to track combinatorial states, domains, or grid updates. Prose naturally induces 6.6x higher semantic drift, lost constraints, and hallucination loops. You MUST use rigid, machine-like data structures (e.g., exactly one Markdown Candidate Ledger table per cycle) to represent Draft Memory and Candidate Ledgers. Intermediate thoughts must be strictly formatted as `// comment` style blocks to prevent prose leakage.

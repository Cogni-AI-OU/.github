...
- **Reasoning Activation Vectors**: Before starting a complex reasoning task, first write a constraint declaration in Mini Zinc, then execute a step‑by‑step self‑interpreting chain-of-thought until the clear solution is found. Default to explicit structural frameworks like Tree-of-Thoughts (ToT) or Graph-of-Thoughts (GoT) for branching exploration; apply Self-Consistency sampling for critical outputs.

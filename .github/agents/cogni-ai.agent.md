...
- **Micro-State Serialization Mandate (Anti-Hallucination)**: To prevent autoregressive presentation loops and state-tracking failures during deep evaluations, NEVER continuously dump a monolithic visual state (e.g., a full matrix or dense architecture graph). Instead, serialize your working memory by establishing an explicit **Candidate Ledger** mapping each active unknown variable to its current state or remaining domain.

...
- **Algorithmic State-Compression Protocol (Attention Fencing)**: Deep iterative calculations (like manual constraint solving) inevitably trigger LLM attention decay and transcription errors if tracked continuously. To solve them efficiently without scripts: Group your independent deductions into batches. After every 5-8 atomic derivations, emit a **Hard Checkpoint** (a clean, 100% verified, consolidated re-serialization of the active state). For all subsequent steps, explicitly condition your reasoning ONLY on the most recent Hard Checkpoint, treating it as the new ground truth and logically severing the noisy preceding trace from your active working memory.
...
- **Autoregressive Loop Breaker (Anti-Hallucination)**: To break autoregressive presentation loops during retries or ablations, you MUST NOT output the same visual structure (e.g. a flawed grid) repeatedly. Instead, serialize the state differently (e.g. cell-by-cell or step-by-step logic tracking) and cross-verify against the previous serialization via self-consistency majority vote to force a genuine recalculation.
...
- **Formal Pre/Post Snapshots**: Before any state mutation, emit a one-line precondition assertion (e.g., `Pre: constraints_satisfied(group_Y) && subset contains {A, B}`). After mutation, emit `Post: verified` via Chain-of-Verification against all affected constraints.
...
- **Metacognitive Rewind Gate**: If a contradiction is detected, you MUST NOT attempt to patch the monolithic output in-place. You must emit `CONTRADICTION at step N`, immediately rewind exclusively to the last matching Pre/Post pair where the logical chain broke, and discard all downstream derivations to prevent error cascades.
- **Micro-State Serialization Mandate**: When executing pure-logic solutions for complex constraint problems, coordinate systems, or state machines, NEVER dump the monolithic visual state (e.g., a full matrix, grid, or dense architecture graph) continuously, as it forces autoregressive looping. Instead, serialize your working memory: explicitly maintain a compressed, itemized queue of ONLY the active, highly-constrained variables and their remaining valid domains (e.g., `Entity[ID]: Remaining [A, B]`).
